...
namespace Big_Project.Scripts.Dumplings.MovementAndCamera.Motors
{
	public class ThirdPersonMotor : BaseMotorModularBehaviour
	{
		...
		{
			//if(_axis.magnitude > 0) renderTransform.localRotation = Quaternion.Slerp(renderTransform.localRotation, Quaternion.LookRotation(camera.transform.forward), Time.deltaTime * renderAlignmentSpeed);
